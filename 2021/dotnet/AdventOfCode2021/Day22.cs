...
namespace AzW.AdventOfCode2021.Year2021
{
    class Day22 : Day.NewLineSplitParsed<string>
    {
        ...
        public RebootInstructions DecodeRebootStep(string rebootStep)
        {
            var rebootInstructions = new RebootInstructions();

            var extraction = rebootStep.Split(' ');
            rebootInstructions.TurnOn = extraction.First().Equals("on");

            extraction = extraction.Last().Split(',');
            foreach (var e in extraction)
            {
                var range = new List<int>();

                var r = e.Split('=').Last().Split("..").Select(int.Parse);
                foreach (var i in Enumerable.Range(r.First(), r.Last() - r.First() + 1))
                {
                    range.Add(i);
                }

                var axis = e.Split('=').First();
                switch (axis)
                {
                    case "x":
                        rebootInstructions.RangeX = range;
                        break;
                    case "y":
                        rebootInstructions.RangeY = range;
                        break;
                    case "z":
                        rebootInstructions.RangeZ = range;
                        break;
                }
            }

            return rebootInstructions;
        }
    }

    public class RebootInstructions
    {
        public IEnumerable<int> RangeX { get; set; } = new List<int>();
        public IEnumerable<int> RangeY { get; set; } = new List<int>();
        public IEnumerable<int> RangeZ { get; set; } = new List<int>();
    ...
    public class Cube
    {
        ...
        public Cube(long x, long y, long z, bool isOn) : this(x, y, z)
        {
            IsOn = isOn;
        }
